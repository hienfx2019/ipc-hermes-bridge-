...
# MVP: lưu job tạm trong RAM.
# Sau này sẽ lưu vào DB của IPC Land hoặc database riêng.
...
class ResearchKCNRequest(BaseModel):
    ...
    source_url: Optional[str] = Field(
        None,
        examples=["https://ipcland.vn/khu-cong-nghiep/khu-cong-nghiep-vsip-quang-ngai"]
    )
    note: Optional[str] = Field(
        None,
        examples=["Lấy quy mô, chủ đầu tư, logistics, ngành phù hợp"]
    )
    callback_url: Optional[str] = Field(
        None,
        examples=["https://ipcland.vn/api/hermes/callback"]
    )
...
def verify_auth(authorization: Optional[str]):
    if not API_KEY:
        raise HTTPException(
            status_code=500,
            detail="Server missing IPC_HERMES_API_KEY"
        )
...
async def fake_research_kcn(job_id: str, payload: ResearchKCNRequest):
    """
    Đây là bản demo.
    Sau này mình sẽ thay hàm này bằng logic Hermes/OpenRouter/Web Research thật.
    """

    JOBS[job_id]["status"] = "running"
    JOBS[job_id]["updated_at"] = now_iso()

    # Giả lập xử lý mất 3 giây
    ...
    result = {
        ...
        "summary": f"Bản nháp dữ liệu cho {payload.kcn_name}. Cần admin IPC Land kiểm tra trước khi lưu DB.",
        "data": {
    ...
    # Nếu IPC Land gửi callback_url thì Hermes Bridge gọi trả kết quả về
